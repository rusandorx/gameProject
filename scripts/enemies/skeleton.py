...
class SkeletonSpearman(CombatEnemy):
    def __init__(self, name, lvl, hp, stats, position, player, *groups):
        super().__init__(name, lvl, hp, stats, position, player, *groups)
        self.stats["attack"] *= lvl
        self.particle.offset = (-15, 110)
    ...
    def load_assets(self):
        ...
        graphics_path = '../graphics/ui/combat/sprites/Skeleton_Spearman/'
        self.sprites = {
            'idle': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Idle.png')).load_strip(
            ...
            'hurt': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Protect.png')).load_strip(
                        pygame.Rect(0, 0, 128, 128), 2, (0, 0, 0)))),
            'die': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Dead.png')).load_strip(
    ...
    def bleeding_attack(self, target=None):
        ...
        if is_bleeding:
            target.add_effect(
                BleedingEffect({'name': 'bleeding', 'turn_count': 1, 'color': (0, 0, 0), 'bleeding_value': .05}))


class SkeletonWarrior(CombatEnemy):
    def __init__(self, name, lvl, hp, stats, position, player, *groups):
        super().__init__(name, lvl, hp, stats, position, player, *groups)
        self.stats["attack"] *= lvl
        self.particle.offset = (-15, 110)
    ...
    def load_assets(self):
        ...
        graphics_path = '../graphics/ui/combat/sprites/Skeleton_Warrior/'
        self.sprites = {
            'idle': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Idle.png')).load_strip(
            ...
            'hurt': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Protect.png')).load_strip(
                        pygame.Rect(0, 0, 128, 128), 1, (0, 0, 0)))),
            'die': list(
                map(lambda sprite: pygame.transform.flip(pygame.transform.scale(sprite, (512, 512)), True, False),
                    SpriteSheet(os.path.join(graphics_path, 'Dead.png')).load_strip(
                        pygame.Rect(0, 0, 128, 128), 4, (0, 0, 0)))),
    ...
    def attack(self, target=None):
        ...
        if self.player.player.mp <= self.stats["manaburn"]:
            ...
            self.player.take_damage(self.stats["manaburn"], "net")
        self.player.take_damage(self.stats["attack"])